...
with header:
    ...
    st.markdown("Prizepicks has countless sports picks playing at a given time. Lots of the plays on Prizepicks are being offered on other sportsbooks, and we can use that to our advantage.\
        Every play on Prizepicks is **assumed to be 50/50**, but that's impossible for every line. You can go to a sportsbook (Draftkings in this case) and compare the odds.")
...
def highlight(s):
    is_plus = s == 'Under +'
    return ['background-color: #3EAAFF' if v else '' for v in is_plus]
...
#     # .apply(highlight2))
    ...
    st.dataframe(filtered_df.style.highlight_quantile(axis=0, subset='Probability', color='#3DCC8C', q_right=1, q_left=0.8)\
